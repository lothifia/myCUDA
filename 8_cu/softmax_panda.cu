...
__global__ void SoftmaxWarpImpl(const int64_t rows, const int64_t cols, const T *x, T *y)
{
    ...
    constexpr int num_packs = cols_per_thread / pack_size; // 每个T 向量处理 pack_size， num_packs 为需要的pack
...
#pragma unroll
                for (int i = 0; i < pack_size; ++i)
                {
                    buf[pack_id * pack_size + i] = -Inf<ComputeType>();
                }
            }
        }
        ...
        for (int row_id = 0; row_id < rows_per_thread; ++row_id)
        {
            ...
            for (int pack_id = 0; pack_id < num_packs; ++pack_id)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < pack_size; ++i)
                    {
                        row_buf[pack_offset + i] = -Inf<float>();
                    }
                }
            }
        }
